...
def get_ad_unit_by_name(ad_unit_name):
  """
  Gets an ad unit by name from DFP.
  ...
  inventoryService = dfp_client.GetService('InventoryService',
    version='v201802')
  ...
  response = inventoryService.getAdUnitsByStatement(
  ...
  if no_ad_unit_found:
    raise DFPObjectNotFound('No DFP ad unit found with name {0}'.format(
      ad_unit_name))
  else:
    ad_unit = response['results'][0]
    logger.info(u'Found ad unit with name "{name}".'.format(name=ad_unit['name']))
  return ad_unit
